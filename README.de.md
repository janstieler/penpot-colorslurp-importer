...
Den Inhalt von `dist/` auf einem beliebigen Webserver bereitstellen. `VITE_BASE_URL` in `.env.production` auf die eigene Domain setzen und `npm run build` ausführen.
...
| Berechtigung | Grund |
|---|---|
| `content:read` | Benötigt für Penpot Event-Listener (Theme-Wechsel) |
